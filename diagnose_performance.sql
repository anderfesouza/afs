...
-- ============================================================
-- Execute essas queries para entender os gargalos
-- ============================================================
...
SELECT
  ...
  COUNT(DISTINCT id_cliente, id_categoria, metodo_pagamento) AS total_contratos,
  MIN(vencimento) AS primeira_data,
  MAX(vencimento) AS ultima_data
FROM dagiel67_central_mdzd.ContaAzulReceitas;

-- 3. EXPLAIN da query (cole a query FINAL aqui depois de "EXPLAIN")
-- Isso mostrará o plano de execução
EXPLAIN
SELECT
  a.id_cliente,
  a.nome_cliente,
...
-- 4. Verificar cardinality dos índices
SELECT
  INDEX_NAME,
  CARDINALITY,
  SEQ_IN_INDEX,
